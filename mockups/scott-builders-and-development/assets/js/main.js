// Scott Builders and Development LLC — interactions
// Load choreography, elevation-drawing line draw, scroll reveals, magnetic
// CTAs, board tilt, scroll progress, footer signature fitting, brand favicon.
// Every motion path is gated behind prefers-reduced-motion: the `is-anim`
// class (set here, only when motion is allowed) is what arms the CSS.
...
(() => {
  ...
  /* Load choreography — CSS stages everything off .is-anim / .is-loaded */
  if (!reduceMotion) {
  ...
  /* Logomark — first letter of the business name in the brand square */
  const logomark = document.querySelector('.logomark');
  if (logomark && name) logomark.textContent = name.charAt(0);
  ...
  if (brand && name) {
    const svg =
      ...
      `<text x="32" y="46" text-anchor="middle" font-family="Arial Narrow,Arial,sans-serif" font-weight="bold" font-size="40" fill="#f4f3ef">${name.charAt(0).toUpperCase()}</text>` +
      `</svg>`;
  ...
  /* Elevation drawing — chalk the lines in on load */
  if (!reduceMotion) {
    document.querySelectorAll('.board-art .draw').forEach((path, i) => {
      const len = path.getTotalLength();
      path.style.strokeDasharray = String(len);
      path.style.strokeDashoffset = String(len);
      path.style.transition = `stroke-dashoffset 1.4s cubic-bezier(0.16, 1, 0.3, 1) ${0.5 + i * 0.07}s`;
      requestAnimationFrame(() => {
  ...
  if (!reduceMotion && finePointer) {
    ...
    /* Board tilt — quiet parallax, max ~2.5 degrees */
