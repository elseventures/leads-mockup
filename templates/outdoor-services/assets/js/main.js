/* Magnolia Grounds Co. — interaction layer. Zero dependencies. */
...
(() => {
  ...
  const seen = sessionStorage.getItem("mgc-intro") === "1";
  ...
  sessionStorage.setItem("mgc-intro", "1");
...
/* ------------------------------------------------------------
   Google stars light up in sequence
   ------------------------------------------------------------ */
(() => {
  const stars = document.getElementById("googleStars");
  if (!stars) return;
  stars.querySelectorAll("i").forEach((s, i) => s.style.setProperty("--si", i));
